...
func TestWrapperAccountHoldings(t *testing.T) {
	t.Parallel()

	_, err := testWrapper.AccountHoldings(t.Context(), exchName, asset.Spot)
	if err != nil {
		t.Fatal(err)
	}

	_, err = testWrapper.AccountHoldings(t.Context(), exchError.String(), asset.Spot)
	if err == nil {
