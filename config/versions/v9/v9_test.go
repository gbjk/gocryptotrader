...
func TestUpgradeExchange(t *testing.T) {
	...
	_, err := new(v9.Version).UpgradeExchange(t.Context(), []byte(`{"name":"Binance"}`))
	...
	_, err = new(v9.Version).UpgradeExchange(t.Context(), in)
	...
	out, err := new(v9.Version).UpgradeExchange(t.Context(), in)
	require.NoError(t, err)

	out, err = new(v9.Version).UpgradeExchange(t.Context(), out)
	require.NoError(t, err)
...
func TestDowngradeExchange(t *testing.T) {
	t.Parallel()
	in := []byte(`{"name":"Binance","features":{"subscriptions":[{"channel":"1"},{"channel":"2","asset":"all"}]}}`)
	out, err := new(v9.Version).DowngradeExchange(t.Context(), in)
	require.NoError(t, err)
