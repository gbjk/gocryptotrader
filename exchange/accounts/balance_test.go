...
import (
	...
	// "github.com/thrasher-corp/gocryptotrader/common" // For common.NilGuard if used directly in tests
	// "github.com/stretchr/testify/assert" // Example of an assertion library
)
...
// Note: Tests for ProtectedBalance (TestProtectedBalance_GetFree, TestProtectedBalance_reset)
// have been removed because the ProtectedBalance struct definition is missing in the provided
// balance.go source, leading to "undefined: ProtectedBalance" compilation errors from balance.go itself.
// The functions GetFree() and reset() were methods on this undefined type.
// Also, the `balance.notice` field caused a compile error `b.notice undefined (type *balance has no field or method notice)`
// in `balance.go`, which makes `TestInternalBalance_Wait` difficult to test accurately for its intended change-notification behavior.
// The provided tests for `Wait` focus on timeout and cancellation channel mechanics assuming `notice` was somehow present.

// errOutOfSequence and errUpdatedAtIsZero are not exported by balance.go, so we can't directly check for them.
// We rely on expectErr boolean.
var (
	// errOutOfSequence = errors.New("out of sequence") // Example, not usable here
	// errUpdatedAtIsZero = errors.New("updated at timestamp is zero") // Example
)
