...
func (m *Manager) FlushChannels() error {
	...
	for _, cW := range m.connectionManager {
		newSubs, err := cW.generateSubscriptions(newSubs)
		if err != nil {
			return err
		}

		// Case if there is nothing to unsubscribe from and the connection is nil
		if len(newSubs) == 0 && cW.connection == nil {
			continue
		}
		...
		if err = m.updateChannelSubscriptions(cW.connection, cW.subscriptions, newSubs); err != nil {
			return err
		}
