...
var (
	...
	errNoGenerateSubs                 = errors.New("websocket GenerateSubs not set")
...
func (m *Manager) Setup(s *ManagerSetup) error {
	...
	if !m.useMultiConnectionManagement {
		...
		if s.GenerateSubscriptions == nil {
			return fmt.Errorf("%s websocket %w: %w", m.exchangeName, errConnSetup, errNoGenerateSubs)
		}
		if s.DefaultURL == "" {
...
func (m *Manager) connect() error {
	...
	for i, cW := range m.connectionManager {
		...
		err = cW.setup.Connector(context.TODO(), conn)
		if err != nil {
		...
		if cW.setup.Authenticate != nil && m.CanUseAuthenticatedEndpoints() {
			err = cW.setup.Authenticate(context.TODO(), conn)
			if err != nil {
		...
		err = cW.setup.Subscriber(context.TODO(), conn, connSubs)
		if err != nil {
