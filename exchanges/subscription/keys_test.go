...
// TestSubsetKeyMatch exercises the SubsetPairsKey MatchableKey interface implementation
// Given A.Match(B):
// Where A is the incoming key, and B is each key in the store
// Ensures A.Pairs must be a subset of B.Pairs
func TestSubsetPairsKeyMatch(t *testing.T) {
	t.Parallel()

	key := &SubsetPairsKey{&Subscription{Channel: TickerChannel}}
	try := &SubsetPairsKey{&Subscription{Channel: OrderbookChannel}}
