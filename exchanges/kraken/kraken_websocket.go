...
func (k *Kraken) wsHandleData(respRaw []byte) error {
	if strings.HasPrefix(string(respRaw), "[") {
		...
		return k.wsReadDataResponse(cName, pair, msg)
	}
...
func (k *Kraken) wsReadDataResponse(cName string, pair currency.Pair, response []any) error {
	switch cName {
	case krakenWsTicker:
	...
	channelType := strings.TrimRight(cName, "-0123456789")
	switch channelType {
	case krakenWsOHLC:
		return k.wsProcessCandles(cName, response, pair)
	case krakenWsOrderbook:
		return k.wsProcessOrderBook(cName, response, pair)
	default:
		return fmt.Errorf("received unidentified data for subscription %s: %+v", cName, response)
	}
}
...
func (k *Kraken) wsProcessOrderBook(cName string, response []any, pair currency.Pair) error {
	...
	if s == nil {
		return fmt.Errorf("%w: %s %s %s", subscription.ErrNotFound, asset.Spot, cName, pair)
	}
...
func (k *Kraken) wsProcessCandles(cName string, response []any, pair currency.Pair) error {
...
func (k *Kraken) manageSubs(op string, subs subscription.List) error {
	...
	resps, err := conn.SendMessageReturnResponses(r.RequestID, r, len(s.Pairs))

	if err != nil && errors.Is(err, stream.ErrSignatureTimeout) { // Ignore a timeout, because we'll track individual subscriptions
...
func (k *Kraken) wsProcessSubStatus(resp []byte) {
	...
	cName, err := jsonparser.GetUnsafeString(resp, "channelName")
	if err != nil {
		return
	}
	if err = k.getRespErr(resp); err != nil {
...
func (k *Kraken) wsAddOrder(request *WsAddOrderRequest) (string, error) {
	...
	request.RequestID = k.Websocket.AuthConn.GenerateMessageID(false)
	request.Event = krakenWsAddOrder
	request.Token = authToken
	jsonResp, err := k.Websocket.AuthConn.SendMessageReturnResponse(request.RequestID, request)
	if err != nil {
		return "", err
	}
