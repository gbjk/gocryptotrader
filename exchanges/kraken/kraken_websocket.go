...
func (k *Kraken) wsHandleData(respRaw []byte) error {
	if strings.HasPrefix(string(respRaw), "[") {
		...
		c := k.Websocket.GetSubscription(subscription.DefaultKey{Channel: channelName, Pair: wsPair, Asset: asset.Spot})
		if c == nil {
...
func (k *Kraken) wsReadDataResponse(c *subscription.Subscription, response WebsocketDataResponse) error {
	switch c.Channel {
	case krakenWsTicker:
...
func (k *Kraken) wsProcessTickers(c *subscription.Subscription, data map[string]interface{}) error {
...
func (k *Kraken) wsProcessSpread(c *subscription.Subscription, data []interface{}) {
	if len(data) < 5 {
...
func (k *Kraken) wsProcessTrades(c *subscription.Subscription, data []interface{}) error {
...
func (k *Kraken) wsProcessOrderBook(c *subscription.Subscription, data map[string]interface{}) error {
	...
	if asksExist || bidsExist {
		...
		if err != nil {
			outbound := c.Pair // Format required "XBT/USD"
			outbound.Delimiter = "/"
	...
	if !askSnapshotExists && !bidSnapshotExists {
		return fmt.Errorf("%w for %v %v", errNoWebsocketOrderbookData, c.Pair, c.Asset)
	}
...
func (k *Kraken) wsProcessOrderBookPartial(c *subscription.Subscription, askData, bidData []interface{}) error {
...
func (k *Kraken) wsProcessOrderBookUpdate(c *subscription.Subscription, askData, bidData []interface{}, checksum string) error {
	...
	book, err := k.Websocket.Orderbook.GetOrderbook(c.Pair, c.Asset)
	if err != nil {
		return fmt.Errorf("cannot calculate websocket checksum: book not found for %s %s %w", c.Pair, c.Asset, err)
	}
...
func (k *Kraken) wsProcessCandles(c *subscription.Subscription, data []interface{}) error {
...
func (k *Kraken) subscribeToChan(chans []subscription.Subscription) error {
	...
	if err = k.getErrResp(respRaw); err != nil {
		wErr := fmt.Errorf("%w Channel: %s Pair: %s; %w", stream.ErrSubscriptionFailure, c.Channel, c.Pair, err)
...
func (k *Kraken) unsubscribeFromChan(chans []subscription.Subscription) error {
	...
	if err != nil {
		return fmt.Errorf("%w Channel: %s Pair: %s Error: %w", stream.ErrUnsubscribeFailure, c.Channel, c.Pair, err)
	}
	...
	if err = k.getErrResp(respRaw); err != nil {
		wErr := fmt.Errorf("%w Channel: %s Pair: %s; %w", stream.ErrUnsubscribeFailure, c.Channel, c.Pair, err)
		k.Websocket.DataHandler <- wErr
		if e2 := k.Websocket.SetSubscriptionState(&c, subscription.SubscribedState); e2 != nil {
			log.Errorf(log.ExchangeSys, "%s error setting channel to subscribed: %s", k.Name, e2)
		}
...
func (k *Kraken) reqForSub(e string, c *subscription.Subscription) (*WebsocketSubRequest, error) {
...
func ensureChannelKeyed(c *subscription.Subscription, r *WebsocketSubRequest) error {
	key, ok := c.EnsureKeyed().(subscription.DefaultKey)
	if !ok {
...
func depthFromChan(c *subscription.Subscription) (int, error) {
...
func timeframeFromChan(c *subscription.Subscription) (int, error) {
