...
var (
	...
	errChannelPayloadSeq  = errors.New("unknown channel for payload sequence")
...
var channelPayloadOrder = map[asset.Item]map[string][]string{
	...
	asset.Options: {
		optionsContractCandlesChannel:        {"interval", "symbol"},
...
func (e *Exchange) subToJson(s *subscription.Subscription, cName string, a asset.Item, p any) (map[string]any, error) {
	...
	// TODO:: These composite channel names can be abstracted across all assets to reduce duplcation throughout
...
var orderbookPayloadConstraints = map[asset.Item]map[string]orderbookPayloadConstraint{
	asset.Spot: {
		...
		spotOrderbookUpdateChannel: {
			...
			Levels:    []int{20, 100}, // These levels are implicitly the same as the Interval, and won't actaully be sent, but Required for SyncOrderbook
		},
	},
	asset.Futures: {
		futuresOrderbookTickerChannel: {},
