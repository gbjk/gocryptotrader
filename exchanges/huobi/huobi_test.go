...
func TestFormatFuturesPair(t *testing.T) {
	...
	r, err = h.formatFuturesPair(p[0], false)
	require.NoError(t, err)
...
func TestPairFromContractExpiryCode(t *testing.T) {
	...
	for _, cType := range contractExpiryNames {
		...
		require.Truef(t, d.After(n), "%s expiry must be today or after", cType)
		switch cType {
		case "CW", "NW":
			require.True(t, d.Before(n.Add(24*time.Hour*14)), "%s expiry must be within 2 weeks", cType)
		case "CQ", "NQ":
			require.True(t, d.Before(n.Add(24*time.Hour*90*2)), "%s expiry must be within 2 months", cType)
		}
	}
}
