...
// testConn is a minimal mock of websocket.Connection for unit tests.
type testConn struct {
	websocket.Connection
	filter any
}

func (c *testConn) IncomingWithData(any, []byte) bool { return false }
func (c *testConn) MessageFilter() any                { return c.filter }

var spotConn = &testConn{filter: asset.Spot}
...
func BenchmarkWsHandleData(bb *testing.B) {
	...
	for bb.Loop() {
		for x := range lines {
			assert.NoError(bb, e.wsHandleData(context.Background(), spotConn,lines[x]))
		}
	}
}
...
func TestWsTickerUpdate(t *testing.T) {
	...
	err := e.wsHandleData(context.Background(), spotConn,pressXToJSON)
	if err != nil {
		t.Error(err)
	}
}
...
func TestWsDepthUpdate(t *testing.T) {
	...
	if err := e.wsHandleData(context.Background(), spotConn,update1); err != nil {
		t.Fatal(err)
	}
	...
	if err = e.wsHandleData(context.Background(), spotConn,update2); err != nil {
		t.Error(err)
	}
...
func TestWsOrderExecutionReport(t *testing.T) {
	...
	err := e.wsHandleData(context.Background(), spotConn,payload)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWsOutboundAccountPosition(t *testing.T) {
	...
	if err := e.wsHandleData(context.Background(), spotConn,payload); err != nil {
		t.Fatal(err)
	}
}
